...
( function() {
	 ...
	 * Featured image.
	 */
	$tmp            = get_post_thumbnail_id( get_the_ID() );
	$has_thumbnail  = $tmp && get_post( $tmp );
	...
	*/
	...
	?>
		<?php if ( $use_short && $has_thumbnail ) : ?>
			<div class="col-md-<?php echo esc_attr( $thumbnail_columns ); ?>">
		<?php endif; ?>
	...
	?>
		<?php if ( $use_short && $has_thumbnail ) : ?>
			<div class="col-md-<?php echo esc_attr( $content_columns ); ?>">
		<?php endif; ?>
	...
	<div class="<?php echo implode( ' ', $classes ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>">
		...
		<?php if ( $thumbnail_first ) : ?>
			<?php echo $thumbnail_output; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
			<?php echo $content_output; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
		<?php else : ?>
			<?php echo $content_output; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
			<?php echo $thumbnail_output; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
		<?php endif; ?>
