...
function bootswatch_recommend_plugin( $plugin ) {
	add_filter(
		'bootswatch_recommended_plugins',
		function( $recommended_plugins ) use ( $plugin ) {
...
add_action(
	...
	function() {
		 ...
		 */
		$recommended_plugins = apply_filters( 'bootswatch_recommended_plugins', array() );
		foreach ( $recommended_plugins as $plugin ) {
			$message = '';
			if ( is_plugin_active( $plugin['file'] ) ) {
				// Plugin is installed and active.
				continue;
			} elseif ( file_exists( WP_PLUGIN_DIR . '/' . ( $plugin['file'] ) ) ) {
			...
			$message .= str_replace( array( '{{link}}', '{{verb}}', '{{ucf_verb}}' ), array( $link, $verb, $ucf_verb ), $plugin['description'] );
			/**
			 * Display notice.
			 */
